...
Это форк `react-color`, публикуемый как `react-color-x` и уже почти доведённый до современного состояния: TypeScript, актуальные сборка/тесты/Storybook/доки, сохранение совместимости публичного API там, где это возможно без major-релиза.

`AGENTS.md` хранит только актуальные и постоянные правила. Исторические этапы миграции и планы держим в [`PLAN.md`](PLAN.md).
...
Следующая крупная отдельная волна — styling modernization: уход от `reactcss` к более современной styling-системе. Это не делать «заодно» с обычными багфиксами или мелкими улучшениями. Подробности — в [`PLAN.md`](PLAN.md).
