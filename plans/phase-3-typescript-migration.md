...
Фаза 3 по основной цели завершена:

- весь production-код библиотеки в `src/`, включая helper-ы, low-level common-компоненты, `ColorWrap`, picker-компоненты и корневые entrypoints, уже переведён на `.ts`/`.tsx`
- публичный entrypoint [`src/index.ts`](../src/index.ts) экспортирует тот же набор default/named exports, а корневой [`index.d.ts`](../index.d.ts) синхронизирован с результатом сборки
...
- Истории и тесты компонента по возможности переносить в том же пакете работ, чтобы snapshot-ы, Storybook и исходники оставались синхронными.
