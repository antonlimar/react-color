...
Фаза 2 по своей основной цели завершена:

- пакет собирается через `tsc` в `lib/` и `es` с сохранением drop-in layout;
- линтинг переведён на ESLint flat config;
- тесты переведены на Vitest + jsdom + Testing Library;
- Storybook обновлён до современного стека на `@storybook/react-vite`;
...
Оставшиеся после завершения фазы 2 cleanup-задачи относятся уже не к миграции toolchain как таковой, а к post-phase-4 follow-up и поэтому отслеживаются отдельно в [`plans/phase-4-dependencies-and-legacy.md`](./phase-4-dependencies-and-legacy.md).
